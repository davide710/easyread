...
with gr.Blocks(title="EasyRead", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            hidden_highlight_output = gr.Textbox(visible=False)
            hidden_trigger_button = gr.Button("Trigger Highlight Function", visible=False)
    ...
    hidden_trigger_button.click(
        fn=simplify_text,
        inputs=[hidden_highlight_output],
    ...
    demo.load(
        ...
        function setupTextHighlighting() {
            const textbox = document.querySelector('#original_text_area');
            ...
            if (textbox) {
                textbox.addEventListener('mouseup', function() {
                    ...
                    const hiddenOutput = document.querySelector('[data-testid="hidden_highlight_output"] textarea');
                    const hiddenButton = document.querySelector('[data-testid="hidden_trigger_button"] button');

                    if (hiddenOutput && hiddenButton) {
                        hiddenOutput.value = selectedText;
                        // Trigger the change event on the hidden input to ensure Gradio registers the update
                        const event = new Event('change');
                        hiddenOutput.dispatchEvent(event);

                        // Click the hidden button to trigger the Python function
                        hiddenButton.click();
                    }
                });
            } else {
                console.warn("Main textbox element not found for setting up highlighting.");
            }
        }
        setTimeout(setupTextHighlighting, 1000); // Give Gradio components some time to render
        """,
    )
